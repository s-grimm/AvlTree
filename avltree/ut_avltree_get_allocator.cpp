...
BOOST_AUTO_TEST_CASE( ut_get_allocator ) {
	...
	try	{
		...
		avltree<int, int>::allocator_type leAllocator;
		leAllocator = tree.get_allocator();
		avltree<int,int> tree2(std::less<int>(), leAllocator);

		BOOST_CHECK( tree.get_allocator() == tree2.get_allocator() );
	}
	catch(...) {
